...
export function mountEngineers(root) {
  root.innerHTML = "";

  const header = el("div", { class: "role-header" }, [
    el("h2", {}, "Инженерный отдел — чертежи домов"),
    el("p", { class: "muted" }, "Технические чертежи зданий комплекса."),
  ]);
  ...
  function renderTripBar(active, tripData) {
    ...
    if (active) {
      ...
      tripBar.appendChild(
        el("button", { class: "btn primary", onclick: () => startTrip("engineers") }, [
          "В командировку (",
  ...
  unsubTrip = listenTrip("engineers", (active, tripData) => {
  ...
  const board = createDrawBoard({
    canvas,
    path: PATHS.engineeringBlueprint,
    color: "#111827",
  });
